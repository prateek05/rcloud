...
PANDOC_OPTS= --from=markdown_strict --to=html  --css=../../../css/pandoc.css
...
DST =  $(foreach fn,$(SRC), $(subst .md,.html,$(OUT_DIR)/$(fn)))

# Create a list of output directories
DST_DIRS = $(foreach dir,$(DIRS),$(OUT_DIR)/$(dir))
...
DST_IMG_DIRS = $(foreach dir,$(SRC_IMG_DIRS),$(OUT_DIR)/$(dir))

# Create a list of output images
DST_IMG      = $(foreach fn,$(SRC_IMG),$(OUT_DIR)/$(fn))
...
$(foreach fn,$(SRC),$(eval $(call make-goal,$(subst .md,.html,$(OUT_DIR)/$(fn)),$(fn))))
...
$(foreach fn,$(SRC_IMG),$(eval $(call make-goal-img,$(OUT_DIR)/$(fn),$(fn))))
